...
import vm from 'vm';
import {JoiX, checkConditional} from './JoiX';
        ...
        components.forEach((component) => {
            ...
            switch (component.type) {
                ...
                case 'datagrid':
                    ...
                    fieldValidator = JoiX.array().items(JoiX.object().keys(objectSchema))
                ...
                case 'select':
                    if (component.validate && component.validate.select) {
                        fieldValidator = JoiX.any().select(component, submission, this.token,
                            this.async, this.requests);
                    }
            ...
            // if multiple masks input, then data is object with 'value' field,
            // and validation should be applied to that field
            ...
            if (component.multiple) {
                // Allow(null) was added since some text fields have empty strings
                // converted to null when multiple which then
    ...
    public validate(submission, next) {
        ...
        JoiX.validate(submission.data, schema, {stripUnknown: true, abortEarly: false}, (validateErr, value) => {
            ...
            Promise.all(this.async).then((errors) => {
                ...
                // Add in any asynchronous errors.
                if (errors.length) {
                    if (!validateErr) {
                ...
                if (validateErr) {
                    // Remove any conditionally hidden validations.
                    // Joi will still throw the errors but we don't want them since the
                    // fields are hidden.
